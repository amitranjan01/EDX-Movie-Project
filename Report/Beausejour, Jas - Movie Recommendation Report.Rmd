...
For now, we will use the method that follows, which rounds to the nearest half-mark:

$$
Rounded\ Predictions = round(\frac{Predictions}{0.5})*0.5
$$

Turning this into a function:

```{r}
rounding_basic <- function(x){
  round(x/0.5)*0.5
}
```
...
```{r}
rounded_average_rating <- rounding_basic(average_rating)
...
rounded_movie_effect_RMSE <- RMSE(rounding_basic(predicted_ratings),test_set$rating)
rounded_movie_effect_Accuracy <- accuracy(rounding_basic(predicted_ratings),test_set$rating)
...
rounded_movieanduser_effect_RMSE <- RMSE(rounding_basic(predicted_ratings), test_set$rating) 
rounded_movieanduser_effect_Accuracy <- accuracy(rounding_basic(predicted_ratings), test_set$rating) 
...
round_reg_movie_effect_RMSE <- RMSE(rounding_basic(predicted_ratings), test_set$rating) 
round_reg_movie_effect_Accuracy <- accuracy(rounding_basic(predicted_ratings), test_set$rating) 
...
We can see that the improvement from regularization, unfortunately, is not great. Still, we will continue with this approach and re-do our analysis on **user_effect** because still provided some improvements.
...
reg_movieanduser_effect_RMSE <- RMSE(predicted_ratings, test_set$rating) 
reg_movieanduser_effect_Accuracy <- accuracy(predicted_ratings, test_set$rating) 

rmse_results <- bind_rows(rmse_results,
                          data_frame(Method= "With Regularized Movie and User Effect",
                                     RMSE=reg_movieanduser_effect_RMSE,
                                     Accuracy = reg_movieanduser_effect_Accuracy))

rounded_reg_movieanduser_effect_RMSE <- RMSE(rounding_basic(predicted_ratings), test_set$rating) 
rounded_reg_movieanduser_effect_Accuracy <- accuracy(rounding_basic(predicted_ratings), test_set$rating) 

rmse_results <- bind_rows(rmse_results,
                          data_frame(Method= "With Regularized Movie and User Effect (rounded)",
                                     RMSE=rounded_reg_movieanduser_effect_RMSE,
                                     Accuracy = rounded_reg_movieanduser_effect_Accuracy))
...
So far, the last model, where we use regularized movie effects and add the user-specific effect (while rounding the prediction) is the one giving us the greatest accuracy. Unfortunately we are still only at `r paste(round(100*rmse_results[10,3],2),"%",sep="")`. 

At this point, we believe there might be something wrong with the we we are rounding our predictions. Let's try to optimize that.

### Optimizing rounding method using regularized movie effect and user effect

We will now use the **training set** to find the best way to round our predictions. Namely, we know that there are no 0s and that the results cannot be above 5. We also saw that half-marks are much less common than full marks.

Until now, we have been using the method that follows, which rounds to the nearest half-mark:
